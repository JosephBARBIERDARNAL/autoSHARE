...
@pytest.mark.parametrize("input_value,expected_output", [
    (["Don't know", "Refusal", "Not applicable"], pd.Series([pd.NA, pd.NA, pd.NA], dtype="object")),
...
def test_replace_missing_codes(input_value, expected_output):
    ...
    pd.testing.assert_series_equal(modified_df['input'], expected_output, check_names=False)
...
test_cases = [
    (
        pd.DataFrame({'A': [1, 2, 3, 4], 'B': [pd.NA, pd.NA, pd.NA, pd.NA]}),
        0,
        ['B']
    ),
    (
        pd.DataFrame({'A': [1, pd.NA, 3, 4], 'B': [pd.NA, 2, pd.NA, 4]}),
        30,
        ['B']
    ),
    (
