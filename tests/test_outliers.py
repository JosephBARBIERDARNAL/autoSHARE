...
def test_find_z_outliers(synthetic_data, threshold_z, expected_outliers_count):
    ...
    outliers = manager.find_z_outliers(threshold_z, synthetic_data, synthetic_data.columns)
...
def test_find_iqr_outliers(synthetic_data, threshold_iqr, expected_outliers_count):
    manager = OutliersManager()
    outliers = manager.find_iqr_outliers(threshold_iqr, synthetic_data, synthetic_data.columns)
    assert (
        len(outliers) == expected_outliers_count
...
def test_find_outliers_unsupported_method(synthetic_data):
    ...
    with pytest.raises(ValueError) as e:
        manager.find_outliers(
            threshold=1.5,
            method="Unsupported",
            df=synthetic_data,
            numerical_cols=synthetic_data.columns
        )
    assert (
